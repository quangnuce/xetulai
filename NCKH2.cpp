...
struct RotationFuzzy{
	...
	float RightF;
	float LeftBound=-5;
	float AvgLeftBound=-2;
	float AvgRightBound=2;
	float RightBound=5;
}rotation_fuzzy;
...
float defuzzy_rotation(){
	...
	if (rotation_fuzzy.LeftF != 0 || rotation_fuzzy.RightF != 0||rotation_fuzzy.AvgRightF!=0||rotation_fuzzy.AvgLeftF!=0||rotation_fuzzy.MidF!=0)
            {
                rotation_fuzzy.rawValue = (rotation_fuzzy.LeftF * rotation_fuzzy.LeftBound + rotation_fuzzy.RightF * rotation_fuzzy.RightBound+rotation_fuzzy.AvgLeftF*rotation_fuzzy.AvgLeftBound+rotation_fuzzy.AvgRightF*rotation_fuzzy.AvgRightBound+rotation_fuzzy.MidF*0) / (rotation_fuzzy.LeftF + rotation_fuzzy.RightF+rotation_fuzzy.AvgLeftF+rotation_fuzzy.AvgRightF+rotation_fuzzy.MidF);
            }
...
float getMin(float s[],int n) //CHU Y: KHAI BAO MANG DONG**
    {
       float min = 999999999999999999999;
       for(int i = 0; i < n; i++)
        {
            if ( s[i] < min) min = s[i];
        }
        if (min == 999999999999999999999) return 0;
        return min;
    }
...
//    printf(" outputVout_Slow[1]:%f\n ", outputVout_Slow[1]);
    ...
    v.SlowF=getMax(outputVout_Slow, 2);
    v.FastF =getMax(outputVout_Fast,1);
    v.AvgF = getMax(outputVout_Avg, 2);
...
//luat mid
    ...
    rotation_fuzzy.RightF = getMax(outRotation_Right, 6);
